...
class Codec[T, E]:
    ...
    def serialize(
        ...
        An encoder that raises :class:`~pyeffect.panic.PanicError` is a defect,
    ...
    def serialize_unsafe(self, result: Result[T, E]) -> dict[str, object]:
        """Encode ``result``; a serialization error is a defect (:class:`PanicError`)."""
        match self.serialize(result):
            case Ok(envelope):
                return envelope
            case Err(error):
                msg = "serialization failed"
                raise PanicError(msg, cause=error)
    ...
    def deserialize_unsafe(self, data: object) -> Result[T, E]:
        """Decode an envelope; a deserialization error is a defect (:class:`PanicError`).
